...
{
    /*通过成员指针“encoder”找到外层大结构的指针“nec_encoder”从而访问大结构体的其他类型变量*/
...
/**
 * 该函数将结构体rmt_encoder_handle_t的变量nec_encoder进行赋值并返回给参数ret_encoder
 * 该函数还对字节编码器的符号bit0和bit1，以及起始和终止符号根据nec框架编码成对应符号
 */
esp_err_t rmt_new_ir_nec_encoder(const ir_nec_encoder_config_t *config, rmt_encoder_handle_t *ret_encoder)
{
    ...
    nec_encoder->nec_ending_symbol = (rmt_symbol_word_t) {
        ...
        .level1 = 0,
        .duration1 = 0x7FFF,    /*十进制32767us*/
    };

    rmt_bytes_encoder_config_t bytes_encoder_config = {
        .bit0 = {
            .level0 = 1,        /*RMT接收器低电平对应RMT发送器高电平载波1*/
    ...
    /*将大结构体变量nec_encoder的首地址返回给ret_encoder*/
